...
const CAREER_CONTENT_PLACEHOLDER = `커리어 고민을 자유롭게 나눠 주세요.

예시 항목:
- 현재 상황: 어떤 커리어 단계에 있는지 (취준생, 주니어, 이직 준비 중 등)
- 고민 내용: 취업, 이직, 성장에 대한 구체적인 고민
- 시도해본 것: 이미 찾아보거나 시도한 방법들
- 기대하는 것: 어떤 도움이나 조언이 필요한지`;
      ...
      <div className="flex flex-col gap-5 rounded-xl border border-border bg-card p-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder={
              postType === "CAREER"
                ? "예: 신입 개발자로 이직을 준비 중인데 포트폴리오 방향이 고민입니다"
                : "예: React useState 업데이트가 즉시 반영되지 않는 이유가 궁금합니다"
            }
            className={cn(
              ...
              {existingAttachments.map((a) => (
                ...
                >
                  ...
                  {onRemoveExistingAttachment && (
                    <button
                      type="button"
                      onClick={() => onRemoveExistingAttachment(a.url)}
